...
public final class MailUtils {
	...
	public static void sendMail(String toEmail, String emailMsg) throws Exception {
		...
		props.setProperty("mail.host", "localhost");//邮件发送服务器的地址，如果使用的是本地安装的易邮，写成：localhost
		props.setProperty("mail.smtp.auth", "true");//指定验证为true
		Authenticator auth = new Authenticator() {
			public PasswordAuthentication getPasswordAuthentication() {
				return new PasswordAuthentication("zs", "123456");
			}
		};
		Session session = Session.getInstance(props, auth);
		//2_创建一封邮件
		Message message = new MimeMessage(session);
		message.setFrom(new InternetAddress("zs@haozhikeji.com"));
